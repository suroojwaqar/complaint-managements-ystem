...
export default function AdminComplaintForm({ 
  ...
  const handleSubmit = async (data: ComplaintFormData) => {
    try {
      ...
      if (showDepartmentSelection && data.department) {
        const selectedDept = departments.find(d => d._id === data.department);
        toast.success(`Complaint submitted and assigned to ${selectedDept?.name || 'selected department'}!`);
      } else {
        toast.success('Complaint submitted successfully! It will be auto-assigned to the appropriate department.');
      }
    } catch (error) {
      ...
      <CardHeader>
        ...
        <CardDescription>
          {showDepartmentSelection 
            ? 'As an admin, you can assign complaints to specific departments or let the system auto-assign.'
          ...
          <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-6">
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormDescription>
                      Select the category that best describes this complaint
                    </FormDescription>
                    <FormMessage />
                  </FormItem>
                )}
            ...
            {showDepartmentSelection && (
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>Department Assignment (Optional)</FormLabel>
                    <Select onValueChange={field.onChange} defaultValue={field.value}>
                      <FormControl>
                        <SelectTrigger>
                          <SelectValue placeholder="Auto-assign to appropriate department" />
                        </SelectTrigger>
                      </FormControl>
                      <SelectContent>
                        <SelectItem value="">Auto-assign based on nature type</SelectItem>
                    ...
                    <FormDescription>
                      Leave blank to auto-assign, or select a specific department to handle this complaint
                    </FormDescription>
                    <FormMessage />
                  </FormItem>
                )}
            ...
            {/* Auto-assignment Notice for Non-Admins */}
            {!showDepartmentSelection && (
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Textarea
                      placeholder="Please describe the issue in detail, including what happened, when it occurred, and any steps taken..."
                      className="min-h-[120px]"
                      {...field}
                    />
                  </FormControl>
                  <FormDescription>
                    Provide as much detail as possible to help understand and resolve the issue
                  </FormDescription>
                  <FormMessage />
                </FormItem>
              )}
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormDescription>
                      What type of error or issue is being reported?
                    </FormDescription>
                    <FormMessage />
                  </FormItem>
                )}
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground mt-2">
                Upload screenshots, documents, or other files that might help understand the issue
              </p>
            </div>
